...
for (i in 1:length(pollutants)) {
  x = filter(poltot, `Area Type`=='urban', pol==pollutants[i]) %>% .$statistic_value
  y = filter(poltot, `Area Type`=='suburban', pol==pollutants[i]) %>% .$statistic_value
...
for (i in 1:length(pollutants)) {
  x = filter(poltot, `Traffic Type`=='background', pol==pollutants[i]) %>% .$statistic_value
  y = filter(poltot, `Traffic Type`=='traffic', pol==pollutants[i]) %>% .$statistic_value
  temp = t.test(x, y, alternative = "two.sided")$p.value
  assign(pollutants[i], temp)
}
